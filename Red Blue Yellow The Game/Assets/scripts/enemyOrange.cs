...
public class enemyOrange : MonoBehaviour {
	...
	private void playerLifeLoss () {
			lifeloseSound.Play();
			tutorialText.text.text = "Oops! You didn't have the right colors to destroy the spaceship. Now combine 1, 2 & 3 to destroy the white spaceship.";
				lifes.currentLifes--;
				lifes.text.text = "";
	...
	private void addScore (int amount) {
		orangeSound.Play();
		tutorialText.text.text = "Let's get tricky! Try 1, 2 and 3 at the same time now!";
    ...
    void Update () {
    	if (!gm.Instance.Pause) {
	    	...
	    	if (playerHit == true){
	    		playerHit = false;
		    	if (PCKeyboardInput.currentPressedKeys != 5) {
		    			playerLifeLoss();
