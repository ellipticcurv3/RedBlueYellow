...
public class enemyRed : MonoBehaviour {
	...
	private void playerLifeLoss () {
		lifeloseSound.Play();
		tutorialText.text.text = "Oops! You didn't have the right color to destroy the spaceship. Press 2 to destroy the blue spaceship.";
			lifes.currentLifes--;
			lifes.text.text = "";
	...
	private void addScore (int amount) {
		redSound.Play();
		tutorialText.text.text = "Goodjob! Now press 2 to destroy the blue spaceship.";
    ...
    void Update () {
    	if (!gm.Instance.Pause) {
	    	...
	    	if (playerHit == true){
	    		playerHit = false;
		    	if (PCKeyboardInput.currentPressedKeys != 1) {
		    			playerLifeLoss();
